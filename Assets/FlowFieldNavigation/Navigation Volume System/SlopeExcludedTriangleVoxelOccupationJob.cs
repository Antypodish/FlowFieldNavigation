...
[BurstCompile]
internal struct SlopeExcludedTriangleVoxelOccupationJob : IJob
{
    ...
    [ReadOnly] internal NativeArray<int> SlopeExcludedTrigs;
    [ReadOnly] internal NativeArray<HeightTile> HighestVoxelsEachTile;
    [WriteOnly] internal NativeList<int3> CollidedIndicies;
    public void Execute()
    {
        for (int i = 0; i < SlopeExcludedTrigs.Length; i += 3)
        {
            int v1Index = SlopeExcludedTrigs[i];
            int v2Index = SlopeExcludedTrigs[i + 1];
            int v3Index = SlopeExcludedTrigs[i + 2];
                ...
                for (int z = boxMinIndex.z; z <= boxMaxIndex.z; z++)
                {
                    for (int x = boxMinIndex.x; x <= boxMaxIndex.x; x++)
                    {
                        ...
                        if (IsColliding(v1, v2, v3, voxelStartPos, voxelSize))
                        {
                            ...
                            if (voxToCheck.y <= maxY && voxToCheck.y > maxY - depth)
                            {
                                CollidedIndicies.Add(voxToCheck);
                            }
                        }
                    }
                }
