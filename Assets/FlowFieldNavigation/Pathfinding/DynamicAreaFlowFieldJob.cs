...
internal struct DynamicAreaFlowFieldJob : IJobParallelFor
{
    ...
    public void Execute(int index)
    {
        ...
        int startFlowFieldIndex = index;
        int startLocalIndex = (startFlowFieldIndex - 1) % SectorTileAmount;
        int startPickedSector1d = CombinedDynamicAreaFieldReader.FieldIndexToSectorIndex(startFlowFieldIndex, SectorTileAmount);
        ...
        //LOOP DATA
        int curFlowFieldIndex = startFlowFieldIndex;
        int curLocalIndex = startLocalIndex;
        int curPickedSector1d = startPickedSector1d;
        float curIntCost = CombinedDynamicAreaFieldReader.GetIntegrationTile(curFlowFieldIndex).Cost;
        int verDif = 0;
        int horDif = 0;
        ...
        while (curIntCost != 0)
        {
            ...
            int newPickedSector1d = CombinedDynamicAreaFieldReader.FieldIndexToSectorIndex(newFlowFieldIndex, SectorTileAmount);
            float newIntCost = CombinedDynamicAreaFieldReader.GetIntegrationTile(newFlowFieldIndex).Cost;

            if (newIndex.CornerBlockDirection != CornerBlockDirection.None)
            {
                if (lastCornerBlockDirection == CornerBlockDirection.None)
                {
                    float2 curIndexPos = FlowFieldUtilities.LocalIndexToPos(curLocalIndex, curPickedSector1d, SectorMatrixColAmount, SectorColAmount, TileSize, sectorSize, FieldGridStartPos);
            ...
            if (lastCornerBlockDirection != CornerBlockDirection.None)
            {
                float2 newIndexPos = FlowFieldUtilities.LocalIndexToPos(newLocalIndex, newPickedSector1d, SectorMatrixColAmount, SectorColAmount, TileSize, sectorSize, FieldGridStartPos);
        ...
        CombinedDynamicAreaFieldReader.SetFlow(index, flow);
    }
    ...
    NewIndexData GetNextIndex(int localIndex, int pickedSector1d, int originalSector1d, float curIntCost, int horizontalDif, int verticalDif, int2 startGeneral2d, float2 startPos)
    {
        ...
        if (neCorner)
        {
            ...
            if (IsSightCorner(startGeneral2d, curGeneral2d, neGeneral2d))
            {
                ...
                float2 curPos = FlowFieldUtilities.IndexToPos(curGeneral2d, TileSize, FieldGridStartPos);
        ...
        int9 sectorMarks = CombinedDynamicAreaFieldReader.GetSectorFlowStartInThePassedOrder(nSector1d, eSector1d, sSector1d, wSector1d, neSector1d, seSector1d, swSector1d, nwSector1d);
        int nSectorMark = sectorMarks.a;
        int eSectorMark = sectorMarks.b;
        int sSectorMark = sectorMarks.c;
        int wSectorMark = sectorMarks.d;
        int neSectorMark = sectorMarks.e;
        int seSectorMark = sectorMarks.f;
        int swSectorMark = sectorMarks.g;
        int nwSectorMark = sectorMarks.h;
        ...
        if (nSectorMark != 0 && nAdjacent && upAvailable) { nIntCost = CombinedDynamicAreaFieldReader.GetIntegrationTileUnsafe(nSectorMark, nLocal1d).Cost; }
        if (eSectorMark != 0 && eAdjacent && rightAvailable) { eIntCost = CombinedDynamicAreaFieldReader.GetIntegrationTileUnsafe(eSectorMark, eLocal1d).Cost; }
        if (sSectorMark != 0 && sAdjacent && lowAvailable) { sIntCost = CombinedDynamicAreaFieldReader.GetIntegrationTileUnsafe(sSectorMark, sLocal1d).Cost; }
        if (wSectorMark != 0 && wAdjacent && leftAvailable) { wIntCost = CombinedDynamicAreaFieldReader.GetIntegrationTileUnsafe(wSectorMark, wLocal1d).Cost; }
        if (neSectorMark != 0 && nAdjacent && eAdjacent && upAvailable && rightAvailable) { neIntCost = CombinedDynamicAreaFieldReader.GetIntegrationTileUnsafe(neSectorMark, neLocal1d).Cost; }
        if (seSectorMark != 0 && sAdjacent && eAdjacent && lowAvailable && rightAvailable) { seIntCost = CombinedDynamicAreaFieldReader.GetIntegrationTileUnsafe(seSectorMark, seLocal1d).Cost; }
        if (swSectorMark != 0 && sAdjacent && wAdjacent && lowAvailable && leftAvailable) { swIntCost = CombinedDynamicAreaFieldReader.GetIntegrationTileUnsafe(swSectorMark, swLocal1d).Cost; }
        if (nwSectorMark != 0 && nAdjacent && wAdjacent && upAvailable && leftAvailable) { nwIntCost = CombinedDynamicAreaFieldReader.GetIntegrationTileUnsafe(nwSectorMark, nwLocal1d).Cost; }
