...
public struct CollisionCalculationJob : IJobParallelForTransform
{
    ...
    public void Execute(int index, TransformAccess transform)
    {
        ...
        //FOR DIRECTION
        float speed2d = math.length(AgentVelocities[index]);
        float2 dir2d = math.select(0f, AgentVelocities[index] / speed2d, speed2d > 0f);
        ...
        if (wallIndiciesAroundDest.Length > 0)
        {
            ...
            if (GetCollision(WallObjectList[wallIndiciesAroundDest[0]], dest2d, AgentMovementData[index].Radius, out collisionOutput))
            {
                ...
                AgentVelocities[index] = newDirection * speed2d;
            }
        }
    }
